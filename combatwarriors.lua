--[[
    CombatSpy v2.2 SOURCE

    CombatSpy is a lightweight penetration testing tool that logs remote calls.

    Credits:
        exx - basically everything
        shibby - making it work on combat warriors
        Frosty - GUI to Lua
	  Clicks - Publishing to GitHub
]]

-- shuts down the previous instance of CombatSpy
if _G.CombatSpyExecuted and type(_G.CombatSpyShutdown) == "function" then
	print(pcall(_G.CombatSpyShutdown))
end
...
local CombatSpy2 = Instance.new("ScreenGui")
...
CombatSpy2.Name = "CombatSpy2"
CombatSpy2.ResetOnSpawn = false

Background.Name = "Background"
Background.Parent = CombatSpy2
...
_G.CombatSpyCONFIG_MaxRemotes = 500
...
-- whether or not CombatSpy uses 'getcallingscript()' to get the script (default is false because detection)
local useGetCallingScript = false

--- used to enable/disable CombatSpy's keyToString for remotes
...
--- @param args any[]
--- @return string
function CombatSpy:ArgsToString(method, args)
...
--- @param t any[]
--- @return string
function CombatSpy:TableToVars(t)
...
--- @param value any
--- @return string
function CombatSpy:ValueToVar(value, variablename)
...
--- @param value any
--- @return string
function CombatSpy:ValueToString(value)
	return v2s(value)
end

--- Generates the CombatSpy function info
--- @param func function
--- @return string
function CombatSpy:GetFunctionInfo(func)
...
function CombatSpy:GetRemoteFiredSignal(remote)
...
--- @param remote Instance
--- @param f function
function CombatSpy:HookRemote(remote, f)
	assert(typeof(remote) == "Instance", "Instance expected, got " .. typeof(remote))
...
--- @param remote any
function CombatSpy:BlockRemote(remote)
	assert(
...
--- @param remote any
function CombatSpy:ExcludeRemote(remote)
	assert(
		typeof(remote) == "Instance" or typeof(remote) == "string",
		"Instance | string expected, got " .. typeof(remote)
	)
...
--- Prevents remote spam from causing lag (clears logs after `_G.CombatSpyCONFIG_MaxRemotes` or 500 remotes)
function clean()
	local max = _G.CombatSpyCONFIG_MaxRemotes
...
--- Executed when the toggle button (the CombatSpy logo) is hovered over
...
--- Called when mouse enters CombatSpy
function mouseEntered()
	local existingCursor = CombatSpy2:FindFirstChild("Cursor")
	while existingCursor do
		existingCursor:Destroy()
		existingCursor = CombatSpy2:FindFirstChild("Cursor")
	end
	...
	customCursor.Image = ""
	customCursor.Parent = CombatSpy2
	UserInputService.OverrideMouseIconBehavior = Enum.OverrideMouseIconBehavior.ForceHide
	RunService:BindToRenderStep("CombatSpy_CURSOR", 1, function()
		if mouseInGui and _G.CombatSpyExecuted then
			...
			RunService:UnbindFromRenderStep("CombatSpy_CURSOR")
		end
	end)
end
...
function backgroundUserInput(input)
	...
	if input.UserInputType == Enum.UserInputType.MouseButton1 and inResizeRange then
		...
		RunService:BindToRenderStep("CombatSpy_RESIZE", 1, function()
		...
		table.insert(
			...
			UserInputService.InputEnded:Connect(function(inputE)
				if input == inputE then
					RunService:UnbindFromRenderStep("CombatSpy_RESIZE")
				end
			end)
		)
...
function newRemote(type, name, args, remote, function_info, blocked, src, returnValue)
	...
	schedule(function()
		...
		if blocked then
			logs[#logs].GenScript = "-- THIS REMOTE WAS PREVENTED FROM FIRING THE SERVER BY CombatSpy\n\n"
				.. logs[#logs].GenScript
		end
	end)
...
function genScript(remote, args)
	...
	gen = "-- Script generated by CombatSpy - credits to shibby#0001 & exx#9394\n\n" .. gen
	prevTables = {}
	return gen
end
local remotes = {}
...
function t2s(t, l, p, n, vtv, i, pt, path, tables, tI)
	...
	for k, v in pairs(t) do -- iterates over table
		...
		if size > (_G.CombatSpyMaxTableSize or 1000) then
			s = s
				.. "\n"
				.. string.rep(" ", l)
				.. "-- MAXIMUM TABLE SIZE REACHED, CHANGE '_G.CombatSpyMaxTableSize' TO ADJUST MAXIMUM SIZE "
			break
		end
	...
	if keyToString then
		if typeof(v) == "userdata" and getrawmetatable(v) then
			return string.format(
				'"<void> (%s)" --[[Potentially hidden data (tostring in CombatSpy:HookRemote/GetRemoteFiredSignal at your own risk)]]',
				safetostring(v)
			)
		elseif typeof(v) == "userdata" then
...
function formatstr(s, indentation)
	...
	return '"'
		...
		.. (
			reachedMax
				and " --[[ MAXIMUM STRING SIZE REACHED, CHANGE '_G.CombatSpyMaxStringSize' TO ADJUST MAXIMUM SIZE ]]"
			or ""
		)
end
	...
	if not toggle then
		if hookmetamethod then
			...
			if not original then
				warn("CombatSpy: namecall method not found!")
				onToggleButtonClick()
				return
			end
...
function shutdown()
	...
	_G.CombatSpyExecuted = false
end

-- main
if not _G.CombatSpyExecuted then
	local succeeded, err = pcall(function()
		if not RunService:IsClient() then
			error("CombatSpy cannot run on the server!")
		end
		if
			not hookfunction
		...
		then
			...
			shutdown()
			error(
				"This environment does not support method hooks!\n(Your exploit is not capable of running CombatSpy)\nMissing: "
					.. table.concat(missing, ", ")
			)
		end
		_G.CombatSpyShutdown = shutdown
		...
		if syn and syn.protect_gui then
			pcall(syn.protect_gui, CombatSpy2)
		end
		bringBackOnResize()
		CombatSpy2.Parent = --[[gethui and gethui() or]]
			CoreGui
		_G.CombatSpyExecuted = true
	...
	if not succeeded then
		warn(
			"A fatal error has occured, CombatSpy was unable to launch properly.\nPlease DM this error message to @exx#9394:\n\n"
				.. tostring(err)
		)
		CombatSpy2:Destroy()
		hookfunction(remoteEvent.FireServer, originalEvent)
...
newButton("Get Script", function()
	...
	if selected then
		setclipboard(CombatSpy:ValueToString(selected.Source))
		TextLabel.Text = "Done!"
	end
end)
...
newButton("Function Info", function()
	...
	if selected then
		if selected.Function then
			codebox:setRaw(
				"-- Calling function info\n-- Generated by the CombatSpy serializer\n\n" .. tostring(selected.Function)
			)
		end
		TextLabel.Text = "Done! Function info generated by the CombatSpy Serializer."
	end
end)
...
newButton("Exclude (i)", function()
	return "Click to exclude this Remote.\nExcluding a remote makes CombatSpy ignore it, but it will continue to be usable."
end, function()
	if selected then
...
newButton("Block (i)", function()
	return "Click to stop this remote from firing.\nBlocking a remote won't remove it from CombatSpy logs, but it will not continue to fire the server."
end, function()
	if selected then
...
newButton("CallingScript", function()
	return string.format(
		"[%s] [UNSAFE] Uses 'getcallingscript' to get calling script for Decompile and GetScript. Much more reliable, but opens up CombatSpy to detection and/or instability.",
...
newButton("GetReturnValue", function()
	...
	if selected then
		codebox:setRaw(CombatSpy:ValueToVar(selected.ReturnValue, "returnValue"))
	end
end)
